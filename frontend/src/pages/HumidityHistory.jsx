// Modern Humidity History Page - Optimized for better visibility
import { useEffect, useState } from "react";
import { fetchMeasurements } from "../api/sensorApi";
import HumidityChart from "../charts/HumidityChart";
import { exportToCsv } from "../utils/exportCsv";
import PageWrapper from "../components/ui/PageWrapper";
import { Droplets, Download, Calendar, Filter, TrendingUp } from "lucide-react";
...
export default function HumidityHistory() {
    ...
    const stats = filteredData.length > 0 ? {
        avg: (filteredData.reduce((sum, m) => sum + m.humidity, 0) / filteredData.length).toFixed(1),
        min: Math.min(...filteredData.map(m => m.humidity)).toFixed(1),
        max: Math.max(...filteredData.map(m => m.humidity)).toFixed(1),
        count: filteredData.length
    } : null;

    return (
        <PageWrapper
            title="Historique Humidité"
                ...
                <div className="flex flex-wrap items-end gap-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 text-white font-medium rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            <Filter size={18} />
                            Filtrer
                        </button>
            ...
            {stats && (
                ...
                >
                    ...
                    <div className="glass-card p-4">
                        ...
                        <div className="text-2xl font-bold text-blue-400">{stats.max}%</div>
                    </div>
                    <div className="glass-card p-4">
